CREATE TABLE ACTIVITE_ANNEXE (
  ...
  constraint FIN_APRES_DEBUT_ACT CHECK (dateDebutActAnn > dateFinActAnn)
);
...
CREATE TABLE PHOTO (
  ...
  idGr int,
  photo blob,
  constraint PK_PHOTO PRIMARY KEY (idPhoto, idGr)
);
...
CREATE TABLE CONCERT (
  ...
  constraint FIN_APRES_DEBUT_CONCERT CHECK (dateDebutConcert > dateFinConcert),
  constraint DEBUT_CONCERT_ENTRE_14H_4H CHECK ((TIME(dateDebutConcert) >= STR_TO_DATE('14:00:00', '%H:%i:%s')) OR (TIME(dateDebutConcert) <= STR_TO_DATE('04:00:00', '%H:%i:%s'))),
  constraint FIN_CONCERT_ENTRE_14H_4H CHECK ((TIME(dateFinConcert) >= STR_TO_DATE('14:00:00', '%H:%i:%s')) OR (TIME(dateFinConcert) <= STR_TO_DATE('04:00:00', '%H:%i:%s')))
);
...
ALTER TABLE ACTIVITE_ANNEXE ADD FOREIGN KEY (idLieu) REFERENCES LIEU (idLieu);
...
ALTER TABLE DEPLACER ADD FOREIGN KEY (idLieuDepart) REFERENCES LIEU (idLieu);
ALTER TABLE DEPLACER ADD FOREIGN KEY (idLieuArrivee) REFERENCES LIEU (idLieu);
...
begin
  ...
  SELECT dateFinConcert into dateDernierConcert FROM CONCERT WHERE idGr = idGroupe AND dateFinConcert > heure ORDER BY dateFinConcert LIMIT 1;
  SELECT dateFinActAnn into dateDerniereActAnn FROM ACTIVITE_ANNEXE NATURAL JOIN PARTICIPER WHERE idGr = idGroupe AND dateFinActAnn > heure ORDER BY dateFinActAnn LIMIT 1;
...
begin
  ...
  SELECT dateDebutActAnn into dateProchaineActAnn FROM ACTIVITE_ANNEXE NATURAL JOIN PARTICIPER WHERE idGr = idGroupe AND dateDebutActAnn < heure ORDER BY dateDebutActAnn DESC LIMIT 1;
...
begin
  ...
  SELECT dateFinConcert, dureeDemontage into dateDernierConcert, demontage FROM CONCERT NATURAL JOIN lieu WHERE idLieu = idL AND dateFinConcert > heure ORDER BY dateFinConcert LIMIT 1;
  SELECT dateFinActAnn into dateDerniereActAnn FROM ACTIVITE_ANNEXE NATURAL JOIN LIEU WHERE idLieu = idL AND dateFinActAnn > heure ORDER BY dateFinActAnn LIMIT 1;
  if dateDernierConcert < dateDerniereActAnn then
...
begin
  ...
  SELECT dateDebutConcert, dureeMontage into dateProchainConcert, montage FROM CONCERT NATURAL JOIN lieu WHERE idLieu = idL AND dateDebutConcert < heure ORDER BY dateDebutConcert DESC LIMIT 1;
  SELECT dateDebutActAnn into dateProchaineActAnn FROM ACTIVITE_ANNEXE NATURAL JOIN LIEU WHERE idLieu = idL AND dateDebutActAnn < heure ORDER BY dateDebutActAnn DESC LIMIT 1;
  if dateProchainConcert < dateProchaineActAnn then
...
begin
  ...
  if dernierOuProchain = 'D' then
    SELECT idLieu into res FROM lieu NATURAL JOIN CONCERT WHERE idGr = idGroupe AND dateFinConcert = heure;
    if res is null then
      SELECT idLieu into res FROM lieu NATURAL JOIN ACTIVITE_ANNEXE NATURAL JOIN PARTICIPER WHERE idGr = idGroupe AND dateFinActAnn = heure;
    end if;
    return res;
  
  -- recherche de l'évènement suivant l'heure donnée
  elseif dernierOuProchain = 'P' then
    SELECT idLieu into res FROM lieu NATURAL JOIN CONCERT WHERE idGr = idGroupe AND dateDebutConcert = heure;
    if res is null then
      SELECT idLieu into res FROM lieu NATURAL JOIN ACTIVITE_ANNEXE NATURAL JOIN PARTICIPER WHERE idGr = idGroupe AND dateDebutActAnn = heure;
    end if;
    return res;
  end if;
  return null;
...
-- Trigger empêchant la duplication de données. Le déplacement entre deux lieu est trié par ordre alphabétique
...
begin
  ...
  SELECT lieu into lieuDepart FROM lieu WHERE idLieu = new.idLieuDepart;
  SELECT lieu into lieuArrivee FROM lieu WHERE idLieu = new.idLieuArrivee;
  if lieuArrivee < lieuDepart then
    set mes = concat('Insertion impossible déplacement entre lieu ', new.idLieuDepart, ' et ', new.idLieuArrivee, '. Inversez les deux lieu pour insérer.');
    signal SQLSTATE '45000' set MESSAGE_TEXT = mes;
  end if;
end|
...
begin
  ...
  set llieu = getIdentifiantLieu(new.idGr, dernierEvenement, 'D');
  if dernierEvenement is not null then
    -- Récupération du temps de trajet entre les deux lieux
    SELECT tempsDeTrajet into dernierTempsTrajet FROM DEPLACER
      WHERE (idLieuDepart = llieu AND idLieuArrivee = new.idLieu) OR (idLieuDepart = new.idLieu AND idLieuArrivee = llieu);
  ...
  set llieu = getIdentifiantLieu(new.idGr, prochainEvenement, 'P');
  if prochainEvenement is not null then
    -- Récupération du temps de trajet entre les deux lieux
    SELECT tempsDeTrajet into prochainTempsTrajet FROM DEPLACER
      WHERE (idLieuDepart = liellieuu AND idLieuArrivee = new.idLieu) OR (idLieuDepart = new.idLieu AND idLieuArrivee = llieu);
...
begin
  ...
  if dernierEvenement is not null then
    if dernierEvenement < new.dateDebutConcert then
      set mes = concat('Insertion concert ', new.idConcert, ' impossible : date trop proche du dernier évènement du lieu ', new.idLieu);
      signal SQLSTATE '45000' set MESSAGE_TEXT = mes;
    end if;
  end if;
  ...
  set prochainEvenement = debutProchainEvenementGroupe(new.idGr, new.dateDebutConcert);
  if prochainEvenement is not null then
    if new.dateFinConcert < prochainEvenement then
      set mes = concat('Insertion concert ', new.idConcert, ' impossible : date trop proche du prochain évènement du lieu ', new.idGr);
      signal SQLSTATE '45000' set MESSAGE_TEXT = mes;
    end if;
  end if;
end|
...
create or replace trigger lieuDisponibleActivite before insert on ACTIVITE_ANNEXE for each row
begin
  declare dernierEvenement datetime;
  declare prochainEvenement datetime;
  declare mes varchar(100);

  set dernierEvenement = finDernierEvenementLieu(new.idLieu, new.dateDebutActAnn);
  if dernierEvenement is not null then
    if dernierEvenement < new.dateDebutActAnn then
      set mes = concat("Insertion de l'activité ", new.idActAnn, ' impossible : date trop proche du dernier évènement du lieu', new.idLieu);
      signal SQLSTATE '45000' set MESSAGE_TEXT = mes;
    end if;
  end if;

  set prochainEvenement = debutProchainEvenementLieu(new.idGr, new.dateDebutActAnn);
  if prochainEvenement is not null then
    if new.dateFinActAnn < prochainEvenement then
      set mes = concat("Insertion de l'activité ", new.idActAnn, ' impossible : date trop proche du prochain évènement du lieu', new.idLieu);
      signal SQLSTATE '45000' set MESSAGE_TEXT = mes;
    end if;
  end if;
end|
